    ...
    db.collection('trucks').remove({}, function(err, result) {
        ...
        db.collection("truck_categories").find().toArray(function(err, result) {
            ...
            result.forEach((item) => {
                ...
                .sort({_id: 1})
                .toArray(function(err, sub1) {
                    if (err) {throw err;}

                    sub1.forEach((sub2) => {
                        console.log(">>> sub1.forEach((sub2)");
                        sub2.image_1_url = handleUrl(sub2.image_1_url);
                        sub2.image_2_url = handleUrl(sub2.image_2_url);
                        sub2.image_3_url = handleUrl(sub2.image_3_url);
                        sub2.image_4_url = handleUrl(sub2.image_4_url);
                        sub2.image_5_url = handleUrl(sub2.image_5_url);
                        sub2.image_6_url = handleUrl(sub2.image_6_url);
                        sub2.image_7_url = handleUrl(sub2.image_7_url);
                        sub2.image_8_url = handleUrl(sub2.image_8_url);

                        db.collection("truck_category_package_items")
                        .find({package_id: sub2._id})
                        .sort({_id: 1})
                        .toArray(function(err, sub3) {
                            if (err) {throw err;}
                            console.log("retrieved truck_category_package_items");
                            sub2.items = sub3;
                        });
                        console.log("<<< sub1.forEach((sub2)");
                    });
